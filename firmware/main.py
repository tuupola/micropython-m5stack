...
a = m5stack.ButtonA(
    callback=lambda pin, pressed: tft.text(
        tft.CENTER, tft.LASTY, "> Button A " + ("pressed.     " if pressed else "released.    ")
    )
)

b = m5stack.ButtonB(
    callback=lambda pin, pressed: tft.text(
        tft.CENTER, tft.LASTY, "> Button B " + ("pressed.     " if pressed else "released.    ")
    )
)
